...
from realtime_fast_system_evoocc_benchmark import (  # noqa: E402
    _load_infos,
    _percentile,
    _safe_avg,
    _select_modes,
...
from realtime_evoocc_runtime import RealtimeNcdeRuntime  # noqa: E402
...
class ModeStats2Hz:
    ...
    evoocc_times_ms: list[float] = field(default_factory=list)
    ...
    def add_step_timing(
        ...
        *,
        ...
        if evoocc_ms is not None:
            self.evoocc_times_ms.append(float(evoocc_ms))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode",
        default="both",
        choices=["fast_only", "fast_plus_evoocc", "both"],
        help="benchmark 模式：仅快系统、快系统+EvoOcc，或顺序跑两者",
    )
    ...
    parser.add_argument("--evoocc-config", required=True, help="evoocc 配置路径")
    parser.add_argument(
        "--evoocc-checkpoint",
        default="",
        help="evoocc 权重路径；fast_plus_evoocc / both 时必填",
    )
    parser.add_argument(
        "--info-path",
        default="",
        help="可选：覆盖 benchmark 使用的 info pkl，默认取 evoocc-config 的 val_info_path",
    )
...
def _run_single_mode_2hz(
    ...
    evoocc_cfg: dict[str, Any],
    evoocc_runtime: RealtimeNcdeRuntime | None,
    *,
    frame_stride: int,
    max_frames: int,
    require_cache_aligned: bool,
    ...
    for info in infos:
        ...
        if opus_runner.get_dataset_index(token) is None:
            ...
            continue
        if mode == "fast_plus_evoocc" and not str(info.get("slow_logit_path", "")):
    ...
    with torch.no_grad():
        for step_batch in dataloader:
            ...
            if current_sample_idx is None or sample_idx != current_sample_idx:
                ...
                if mode == "fast_plus_evoocc":
                    if evoocc_runtime is None:
                        raise RuntimeError("fast_plus_evoocc 模式要求 evoocc_runtime 已初始化。")
                    try:
                        sample_num_frames = evoocc_runtime.begin_sample(current_info)
                    except Exception as exc:
                        current_sample_failed = True
                        stats.add_skip("evoocc_begin_failed")
                        stats.add_skip_example("evoocc_begin_failed", exc)
                    else:
                        if sample_num_frames != int(step_batch["num_steps"]):
                            current_sample_failed = True
                            stats.add_skip("evoocc_step_count_mismatch")
                ...
                if mode == "fast_plus_evoocc":
                    try:
                        cast(RealtimeNcdeRuntime, evoocc_runtime).step(
                            step_idx=0,
                        ...
                        current_sample_failed = True
                        stats.add_skip("evoocc_bootstrap_failed")
                        stats.add_skip_example("evoocc_bootstrap_failed", exc)
                        continue
            ...
            try:
                ...
                if mode == "fast_only":
                    ...
                    step_dense_logits, evoocc_ms = _time_call_ms(
                        lambda: cast(RealtimeNcdeRuntime, evoocc_runtime).step(
                            step_idx=step_idx,
            ...
            total_ms = float(opus_ms + dense_ms + (evoocc_ms or 0.0))
            if collect_results:
    ...
    result = {
        ...
        "avg_ms_evoocc_runtime": _to_json_number(_safe_avg(stats.evoocc_times_ms)),
...
def main() -> None:
    args = parse_args()
    if args.mode in {"fast_plus_evoocc", "both"} and not args.evoocc_checkpoint:
        raise ValueError("fast_plus_evoocc / both 模式必须提供 --evoocc-checkpoint。")

    torch.backends.cudnn.benchmark = True

    evoocc_cfg, all_infos, info_abs = _load_infos(args.evoocc_config, args.info_path)
    ...
    evoocc_runtime = None
    if "fast_plus_evoocc" in modes:
        evoocc_runtime = RealtimeNcdeRuntime(
            config_path=args.evoocc_config,
            checkpoint_path=args.evoocc_checkpoint,
            device=args.device,
        )
    ...
    if warmup_infos:
        for mode in modes:
            _run_single_mode_2hz(
                ...
                evoocc_cfg=evoocc_cfg,
                evoocc_runtime=evoocc_runtime if mode == "fast_plus_evoocc" else None,
    ...
    if "fast_only" in outputs["modes"] and "fast_plus_evoocc" in outputs["modes"]:
        fast_only_avg = outputs["modes"]["fast_only"].get("avg_ms_all_steps", None)
        fast_plus_avg = outputs["modes"]["fast_plus_evoocc"].get("avg_ms_all_steps", None)
        delta_ms = None
        ratio = None
