...
逻辑对齐 ``scripts/eval_evoocc.py``，唯一差别是在喂给模型前用
``SubsampledStepwiseEvalDataset`` 对 fast_logits / 时间戳 / pose 抽帧（保留首尾、
中间任意 gap），从而压力测试 EvoOcc aligner 在非均匀帧率下的表现。
...
示例：
  # 显式
  python tests/evoocc/eval_evoocc_random_interval.py \
...
from evoocc.losses import resize_labels_and_mask_to_logits  # noqa: E402
from evoocc.metrics import MetricMiouOcc3D, apply_free_threshold  # noqa: E402
...
class SubsampledStepwiseEvalDataset(Dataset):
    ...
    def __init__(
        self,
        base_dataset: Occ3DEvoOccDataset,
...
# ---------------------------------------------------------------------------
# sweep pkl 解析（与 eval_evoocc.py 一致）
# ---------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="EvoOcc 抽帧评估（仅末帧 mIoU + RayIoU）",
