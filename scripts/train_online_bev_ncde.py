...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--val-scene-count", type=int, default=40, help="验证集使用的 scene 数量")
...
) -> Occ3DOnlineBevNcdeDataset:
    return Occ3DOnlineBevNcdeDataset(
        info_path=info_path,
        root_path=root_path,
...
def main() -> None:
    ...
    train_dataset = build_dataset(
        ...
        fast_logits_root=fast_logits_root,
        slow_logit_root=slow_logit_root,
        supervision_sidecar_path=data_cfg.get("train_supervision_sidecar_path", "") or None,
    )
    ...
    if args.wandb:
        ...
        run = wandb.init(
            ...
            name=run_name,
            config={"epochs": int(train_cfg["epochs"]), "batch_size": int(train_cfg["batch_size"]), "lr": float(train_cfg["lr"]), "weight_decay": float(train_cfg["weight_decay"])},
        )
    ...
    for epoch in range(start_epoch, int(train_cfg["epochs"]) + 1):
        ...
        if scheduler is not None:
            scheduler.step()
        train_sup_parts = [f"{key}={float(value):.4f}" for key, value in train_metrics.items() if key.startswith("loss_t")]
        train_sup_text = (" " + " ".join(train_sup_parts)) if train_sup_parts else ""
        print(
            f"[train] epoch={epoch} loss={train_metrics['loss']:.4f} focal={train_metrics['focal']:.4f} "
            f"lovasz={train_metrics['aux']:.4f} delta={train_metrics['delta_scene_abs_mean']:.4f}{train_sup_text}"
        )
        if run is not None:
        ...
        if val_loader is not None and args.eval_every > 0 and epoch % args.eval_every == 0:
            ...
            val_sup_parts = [f"{key}={float(value):.4f}" for key, value in val_metrics.items() if isinstance(key, str) and key.startswith("sup_loss_t")]
            val_sup_text = (" " + " ".join(val_sup_parts)) if val_sup_parts else ""
            print(
                f"[eval] epoch={epoch} loss={val_metrics['loss']:.4f} focal={val_metrics['focal']:.4f} "
                f"lovasz={val_metrics['aux']:.4f} miou={val_metrics['miou']:.4f}{val_sup_text}"
            )
