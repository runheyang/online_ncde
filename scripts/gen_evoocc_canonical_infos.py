...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成 evoocc canonical infos。")
    parser.add_argument(
        "--src-info",
    ...
    parser.add_argument(
        "--output",
        default="",
        help="输出 pkl 路径；空则默认写到 configs/evoocc/canonical_infos_<split>.pkl。",
    )
    parser.add_argument(
        "--data-root",
...
def main() -> None:
    ...
    payload = {
        "metadata": {
            "schema_version": "evoocc_canonical_infos_v1",
            "description": (
