#!/usr/bin/env python3
"""生成 EvoOcc 的 start-anchored evolve info pkl。
...
SCHEMA_VERSION = "evoocc_evolve_infos_v1"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="生成 evoocc start-anchored evolve infos。")
    ...
    p.add_argument("--output", default="",
                   help="输出 pkl 路径；空则默认 configs/evoocc/evolve_infos_<split>.pkl")
...
def build_default_output_path(src_info_path: Path) -> Path:
    ...
    return (ROOT / "configs" / "evoocc" / name).resolve()
