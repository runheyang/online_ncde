"""OPUSv1-T fast 系统 + NCDE aligner 的真实流式预测评估.

这是独立入口，不改动 alocc2d_mini 的 streaming 脚本。OPUS 分支只调用
OPUS 模型前向和本项目 mIoU/RayIoU 评估，不触碰 OPUS 自带 dataset.evaluate
里的 RayIoU 逻辑。

示例:
    python scripts/streaming/eval_streaming_realistic_opusv1.py \
        --aligner-cfg configs/online_ncde/fast_opusv1t__slow_opusv2l/base_2hz.yaml \
        --aligner-ckpt ckpts/.../epoch_20.pth \
        --slow-intervals 2.0 \
        --limit-scenes 1
"""
...
def resolve_slow_root(data_cfg: dict) -> str:
    """从 cfg 读取 slow_logit_root，支持绝对路径或相对 REPO_ROOT."""
...
def build_opus_sample_meta_index(meta_pkl_path: str, slow_logit_root: str, gt_root: str) -> dict:
    ...
    OPUS 的 nuscenes_infos_val_sweep.pkl 里 CAM_FRONT 没有 sample_data_token，
    但 streaming 只依赖 sample_token/scene/timestamp/ego/slow/gt，因此 frame_token
    用 data_path basename 兜底即可。这个兼容逻辑只放在 OPUS 专用入口里。
    """
    import pickle
...
def parse_args():
    ...
    p.add_argument("--enable-rayiou", action="store_true",
                   help="兼容旧参数；本脚本默认已启用本项目 RayIoU")
    p.add_argument("--no-rayiou", action="store_true",
                   help="兼容参数；OPUS 分支默认跳过 RayIoU")
...
def stream_scene_one_interval(
    ...
    stream_aligner, data_cfg, slow_interval, device,
    pred_list_out, gt_cache,
):
    """跑一个 scene + interval，收集 (token, pred_uint8)."""
    ...
    for step, ((idx, meta), fl_fp16) in enumerate(zip(kf_list, scene_fast_cache)):
        ...
        if slow_available:
            slow_logits = slow_cache.get(meta.slow_logit_path) if slow_cache is not None \
...
def main():
    ...
    print("\n[5] Phase 1: per-scene OPUS fast + NCDE stream ...")
    t_overall = time.time()
