"""现实在线推理评估 - 慢系统 logits 延迟 1s (2 keyframes @ 2Hz) 场景.

跟 eval_streaming_realistic.py 的差异:
  - 假定快系统 logits 实时, 慢系统 logits 到达时刻 t 携带的内容是 t - delay 时刻的.
  - 前 delay 个 keyframe (默认 2): 没有可用 slow, 直接以 fast logits 作输出.
  - slow 到达 step k (k >= delay) 时:
      * 取 step k - delay 时刻的 fast logits 做差初始化隐藏 (use_fast_residual=True
        时 hidden_{k-delay} = slow_feat - fast_feat_{k-delay}).
      * 用 ego_warp 把 hidden + fast_prev_adv 从 k-delay warp 到 k.
      * 单步 euler 演化 dt = (t_k - t_{k-delay}) ≈ 1s, 不再分步 2Hz.
      * decode + 残差融合 fast_logits_k → 当前时刻输出.
      * 保存 hidden / prev_ego / prev_t_us / prev_fast_feat / prev_tau, 之后只有
        fast 输入的 step 走 stream_aligner.evolve(...) (跟原脚本一样).
  - slow 调度仍按 schedule_slow_steps(ts, slow_interval) 决定 "slow 到达" 帧;
    内容 = kf[step - delay] 对应的 slow_logit_path.

用法:
    python scripts/streaming/eval_streaming_realistic_delayed.py \
        --aligner-cfg configs/online_ncde/fast_alocc2dmini__slow_opusv2l/base_2hz.yaml \
        --aligner-ckpt ckpts/fast_alocc2dmini__slow_opusv2l/.../epoch_20.pth \
        --slow-intervals 1.0 2.0 4.0 \
        --slow-delay-keyframes 2 \
        [--limit-scenes N] --num-workers 4 --preload-slow
"""
...
from online_ncde.streaming.slow_schedule import schedule_slow_steps  # noqa: F401  (兼容直接对照)


def schedule_delayed_slow_steps(timestamps_us, interval_sec: float, delay_kf: int):
    """delayed 场景的 slow 到达调度.

    物理语义: slow 推理在 t=ts[0] 启动, delay_kf 个 keyframe 后(= step=delay_kf)
    第一份结果到达, 之后每 slow_interval 秒一份.

    与 schedule_slow_steps 的差异: 首份 slow 落在 step=delay_kf 而非 step=0,
    避免每 scene 开头 delay_kf..(下一个 interval) 之间整段降级 fast-only.
    """
...
def delayed_reset_and_evolve(
    ...
    *,
    ...
    走 N=curr_step-prev_step 步 2Hz 推演 (每步 dt≈0.5s):
      1. reset_with_slow(fast_prev, slow_prev, ego_prev, t_prev): hidden 落到 prev_step
      2. 对 k = prev_step+1 .. curr_step, 逐步调 stream_aligner.evolve(fast_k, ego_k, t_k)
      3. 最后一次 evolve 的输出即 curr 时刻 aligned logits.

    每个 evolve 内部走 ego_warp + 单步 solver + decode + 残差融合, 多步累计 tau,
    跟训练时 2Hz 多步监督一致.
    """
...
def stream_scene_one_interval_delayed(
    ...
    """delayed slow 注入版本. 前 slow_delay_kf 帧 + 没有 slow 注入前的帧, 输出 = fast.

    slow_steps 仍按 schedule_slow_steps 决定 slow 到达帧;
    slow 描述的内容 = kf[step - slow_delay_kf] 时刻.
    """
    ...
    for step, ((idx, meta), fl_fp16) in enumerate(zip(kf_list, scene_fast_cache)):
        ...
        # delay 之内的帧: 没有可用 slow, 直接输出 fast
        if step < slow_delay_kf:
            ...
            if slow_arrived and slow_present:
                ...
                aligned = delayed_reset_and_evolve(
                    stream_aligner,
                    kf_list=kf_list, scene_fast_cache=scene_fast_cache,
                    prev_step=prev_step, curr_step=step,
...
def main():
    ...
    print(f"\n[5] Phase 1: per-scene 推理 + collect (delayed slow) ...")
    t_overall = time.time()
