...
2. 检查是否有正在运行的训练（检查 GPU 占用：nvidia-smi）
3. 如果有未收集结果的 run（output_dir 下有 metrics.json），收集结果更新 state.json
4. 分析所有历史结果，重点关注：
...
6. 启动训练（后台运行）：
   ```bash
   nohup conda run -n neural_ode python scripts/train_online_ncde_200x200x16.py \
