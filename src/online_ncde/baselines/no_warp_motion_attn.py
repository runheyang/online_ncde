...
ForwardOutput = Dict[str, Union[torch.Tensor, TensorDict]]
...
class _WindowCrossAttention3D(nn.Module):
    ...
    def _to_windows(self, x: torch.Tensor) -> Tuple[torch.Tensor, WindowMeta]:
        B, C, X, Y, Z = x.shape
    ...
    def _from_windows(self, x: torch.Tensor, meta: WindowMeta) -> torch.Tensor:
        B, C, X, Y, Z, _ = meta
        Wx, Wy, Wz = self.window_size
        Sx, Sy, Sz = self.shift_size
        nWx, nWy, nWz = X // Wx, Y // Wy, Z // Wz
    ...
    def forward(self, query: torch.Tensor, key_value: torch.Tensor) -> torch.Tensor:
        ...
        if hasattr(F, "scaled_dot_product_attention"):
            ...
            # PyTorch 1.13 fallback；OPUS 环境没有 fused SDPA。
            scale = float(self.head_dim) ** -0.5
            out = torch.empty_like(q_w)
            chunk_size = 128
            for start in range(0, q_w.shape[0], chunk_size):
                end = min(start + chunk_size, q_w.shape[0])
                q_chunk = q_w[start:end]
                k_chunk = k_w[start:end]
                v_chunk = v_w[start:end]
                attn = torch.softmax(
...
class NoWarpMotionBiasAttnAligner(RecurrentWarpFusionAligner):
    ...
    def _forward_single(
        ...
        frame_timestamps: Optional[torch.Tensor],
        frame_dt: Optional[torch.Tensor],
    ...
    def _forward_single_stepwise_train(
        ...
        delta_mag_values: List[float] = []
        fast_kl_accum: Optional[torch.Tensor] = None
