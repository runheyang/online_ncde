...
from evoocc.data.labels_io import load_labels_npz
from evoocc.data.logits_loader import LogitsLoader
from evoocc.data.ray_sidecar import RaySidecar
    ...
    def __init__(
        ...
        if schema_ver in _EVOLVE_SCHEMAS and not eval_only_mode:
            raise ValueError(
                ...
                "只能用 scripts/eval_evoocc_evolution_times.py（传 eval_only_mode=True）评估。"
                "其他入口（train / eval_evoocc / stepwise）请使用 canonical_infos pkl，"
