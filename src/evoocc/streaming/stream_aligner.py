"""StreamAligner: 把 EvoOccAligner 的逐 step 演化拆成可单步调用的两个 API.
...
复用现有 EvoOccAligner ckpt, 不改模型, 只重构控制流以适配流式场景.
"""
...
from evoocc.data.scene_delta import build_scene_delta_ctrl
...
class StreamAligner:
    ...
    def __init__(self, base_aligner: Any) -> None:
        """base_aligner: EvoOccAligner 已 build + load_state_dict + .eval() 后的实例."""
        self.m = base_aligner
        self.m.eval()
